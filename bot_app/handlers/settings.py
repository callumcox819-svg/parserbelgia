...
async def open_filters(callback: CallbackQuery) -> None:
    ...
    text = (
        ...
        "**Не повторять продавцов** — только **новые люди** в JSON. "
        "Память авто-обрезается (последние **6k** продавцов). "
        "Все категории по кругу; если мало новых — снова с начала. "
        "Если занято — ваш запуск **встанет в очередь**, не жмите повторно.\n\n"
